...
export const SKILL_CATALOG: SkillDefinition[] = [
  ...
  { name: "PRD", pattern: /\bPRD\b|产品需求文档|需求文档撰写/i },
  { name: "用户调研", pattern: /用户调研|用户研究|需求调研|用户访谈/i },
  { name: "产品规划", pattern: /产品规划|产品路线规划|产品路线图/i },
  { name: "原型设计", pattern: /原型设计|产品原型|原型制作|原型图/i },
  { name: "协同开发", pattern: /协同开发|配合开发|推动开发落地|与研发协作|协调研发/i },
...
const responsibilityPattern = /负责|参与|完成|撰写|设计|搭建|输出|制定|规划|推动|协同|落地|配合|协作|协调/;

function sourceSentence(jd: string, pattern: RegExp) {
  const evidenceStrength = (value: string) => interestPattern.test(value) ? 1
    : preferredPattern.test(value) ? 3
    : explicitRequirementPattern.test(value) ? 5
    : responsibilityPattern.test(value) ? 4
    : 2;
  return jd.split(/[。！？!？\n；;]/)
    .map(value => value.trim())
    .filter(value => value && pattern.test(value))
    .sort((a, b) => evidenceStrength(b) - evidenceStrength(a))[0] || "";
}
...
export function buildSkillMatches(input: JobAnalysisInput): SkillMatch[] {
  ...
  return names.map(skill => {
    ...
    const responsibility = responsibilityPattern.test(evidenceText);
    ...
    if (!explicitRequirement && !responsibility) return {
